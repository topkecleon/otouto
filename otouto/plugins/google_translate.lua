...
function tl:init()
    ...
    tl.command = 'translate [lang] \\n <text>'
    tl.doc = self.config.cmd_pat .. [[translate [text]
...
function tl:action(msg)
    ...
    local translate_url = tl.url:format(lang, url.escape(text))
    local result, code = https.request(translate_url)
    if code ~= 200 then
